...
import com.acrylic.universal.entityai.BlockExaminer;
...
public class AStarTest {
   ...
   public void traverse() {
        ...
        do {
            ...
            if (closest == null) {
                Bukkit.broadcastMessage("No new nodes found!");
                break;
            }
            if (closest.equals(end)) {
                computed = closest;
                Bukkit.broadcastMessage("Found with A. ");
                break;
            }
            i++;
   ...
   public void climb(Consumer<Location> action) {
    ...
    public static class Node {
        ...
        public Node(Block start, Block end, Block block) {
            ...
            this.gCost = (float) start.getLocation().distance(location);
            this.hCost = (float) end.getLocation().distance(location);
        }

        public Node(AStarTest aStarTest, Block block) {
            this.location = block.getLocation();
            this.gCost = (float) aStarTest.getStart().getLocation().distance(location);
            this.hCost = (float) aStarTest.getEnd().getLocation().distance(location);
        }
